...
namespace MapProjectorLib.Projections
{
    class Perspective : Transform
    {
        ...
        public static (double sunX, double sunY, double sunZ) CalculateSunCoordinates(TransformParams tParams)
        {
            ...
            double date = tParams.date - 80;
            while (date < 0) date += 365;
            while (date >= 365) date -= 365;
            date = 2 * Math.PI * date / 365;

            double sunHeight = ProjMath.SunHeight(date);
            double q = Math.Sqrt(1 - ProjMath.Sqr(sunHeight));
            // tParams.time is mean time, convert to apparent time
            double eot = ProjMath.EquationOfTime(date) * 2.0 * Math.PI / (60.0 * 60.0 * 24.0);
            // AT = MT + EOT
            double atime = tParams.time + eot;
        ...
        public override void Init(TransformParams tParams)
        {
            ...
            (sunx, suny, sunz) = Perspective.CalculateSunCoordinates(tParams);
                      ...
                      ref double phi, ref double lambda)
        {
            ...
            double qa = a2 * ProjMath.Sqr(rx - x1) + b2 * ProjMath.Sqr(ry - y1) + c2 * ProjMath.Sqr(rz - z1);
            double qb = 2 * (a2 * x1 * (rx - x1) + b2 * y1 * (ry - y1) + c2 * z1 * (rz - z1));
            double qc = a2 * ProjMath.Sqr(x1) + b2 * ProjMath.Sqr(y1) + c2 * ProjMath.Sqr(z1) - 1;
            ...
            if (qm >= 0)
            {
                ...
                else
                {
                    ...
                    double r = Math.Sqrt(ProjMath.Sqr(a2 * x) + ProjMath.Sqr(b2 * y));
                 ...
                 double phi, double lambda, ref double x, ref double y)
        {
            ...
            else
            {
                ...
                // The actual vector
                double r = Math.Sqrt(1 / (ia2 * ProjMath.Sqr(nx) + ib2 * ProjMath.Sqr(ny) + ic2 * ProjMath.Sqr(nz)));
                x0 = nx * r * ia2;
                y0 = ny * r * ib2;
                z0 = nz * r * ic2;
                // And apply rotations
        ...
        protected override Rgb24 AdjustOutputColor(Rgb24 inputcolor, 
            ...
            TransformParams tParams)
        {
            ...
            double sunAltitude = sunx * xProjected + suny * yProjected + sunz * zProjected;
