...
namespace MapProjectorLib
{
    public abstract class Transform
    {
        ...
        public static Transform GetTransform(MapProjection projection)
        {
            switch (projection)
            {
                ...
                case MapProjection.Hammer:
                    return new Hammer();
                default:
                    throw new ArgumentException("Projection Not Supported", nameof(projection));
            }
        }
        ...
        public abstract bool Project(TransformParams tParams,
            double x, double y,
            ref double x1, ref double y1, ref double z1,
		    ref double phi, ref double lambda);

        public abstract bool ProjectInv(TransformParams tParams,
        ...
        public virtual (int w, int h) AdjustSize(int w, int h, TransformParams tParams)
        {
            return (w, h);
        }
        ...
        protected virtual Rgb24 AdjustOutputColor(Rgb24 inputcolor, double x, double y, double z, TransformParams tParams)
        {
            return inputcolor;
        }
        ...
        void SetData(Image inImage, Image outImage,
                    ...
                    int ox, int oy)
        {
            ...
            double halfWidth = imgWidth * 0.5;
            double halfHeight = imgHeight * 0.5;
            double scaledWidth = imgWidth * ProjMath.OneOverTwoPi;
            double scaledHeight = imgHeight * ProjMath.OneOverPi;
            ...
            var outColor = AdjustOutputColor(inImage[scaledX, scaledY], x, y, z, tParams);
            
            outImage[ox,oy] = outColor;
        }

        public bool IsPointWithinRadius(TransformParams tParams, double phi, double lambda)
        {
            if (tParams.radius == 0) return true;

            return (Distance(phi, lambda, tParams.lat, tParams.lon) <= tParams.radius);
        }
        ...
        public void TransformImage(Image inImage, Image outImage,
		    TransformParams tParams)
        {
    
            int outWidth = outImage.Width;
            int outHeight = outImage.Height;
            double xOrigin = 0.5 * outWidth;
            double yOrigin = 0.5 * outHeight;
    
            // Scale so that half width is 1
            double scaleFactor = BasicScale(outWidth, outHeight) / tParams.scale;
        
            for (var outY = 0; outY < outHeight; outY++)
            {
                double y = scaleFactor * (yOrigin - outY - 0.5) + tParams.yOffset;
                if (tParams.rotate == 0) SetY(y);
    
                for (var outX = 0; outX < outWidth; outX++)
                {
                    // Really ought to just apply a 3x2 matrix to the point
                    double x = scaleFactor * (outX + 0.5 - xOrigin) + tParams.xOffset;
                    double x1 = x;
                    double y1 = y;
                    ...
                    if (
                        Project(tParams, x1, y1, ref x0, ref y0, ref z0, ref phi, ref lambda)
                        && IsPointWithinRadius(tParams, phi, lambda))
                    {
                        SetData(inImage, outImage, tParams, x0, y0, z0, phi, lambda, outX, outY);
                    }
        ...
        public void TransformImageInv(Image inImage, Image outImage,
				      TransformParams tParams)
        {
            ...
            for (var oy = 0; oy < oh; oy++)
            {
                for (var ox = 0; ox < ow; ox++)
                {
                    ...
                    double phi = (orgy - oy - 0.5) * Math.PI / oh;
                    double lambda = (ox + 0.5 - orgx) * ProjMath.TwoPi / ow;
        ...
        protected bool SetDataInv(Image inImage, Image outImage,
			       TransformParams tParams,
                   ...
                   )
        {
            int imgWidth = inImage.Width;
            int imgHeight = inImage.Height;
            double xOrigin = 0.5 * imgWidth;
            double yOrigin = 0.5 * imgHeight;

            // Scale so that half width is 1
            double scaleFactor = BasicScale(imgWidth, imgHeight) / tParams.scale;
            ...
            int ix = (int)(Math.Floor(xOrigin + (x - tParams.xOffset) / scaleFactor));
            int iy = (int)(Math.Floor(yOrigin - (y - tParams.yOffset) / scaleFactor));
        ...
        public virtual bool MapXY(Image outImage,
		          TransformParams tParams,
                  double phi, double lambda, 
                  ref double x, ref double y)
        {
                ...
                ProjectInv(tParams, phi, lambda, ref x, ref y))
            {
                ...
                int outImgWidth = outImage.Width;
                int outImgHeight = outImage.Height;

                double xOrigin = 0.5 * outImgWidth;
                double yOrigin = 0.5 * outImgHeight;
                double scaleFactor = BasicScale(outImgWidth, outImgHeight) / tParams.scale;
        ...
        protected void ConvertLatLong(ref double phi, ref double lambda,
                double x, double y, double z,
                Matrix3 m)
        {
        ...
        protected double Distance(double phi0, double lambda0, double phi1, double lambda1)
        {
            double x0 = Math.Cos(lambda0) * Math.Cos(phi0);
            double y0 = Math.Sin(lambda0) * Math.Cos(phi0);
            double z0 = Math.Sin(phi0);
            double x1 = Math.Cos(lambda1) * Math.Cos(phi1);
            double y1 = Math.Sin(lambda1) * Math.Cos(phi1);
            double z1 = Math.Sin(phi1);
            double d = Math.Acos(x0 * x1 + y0 * y1 + z0 * z1);
        ...
        void DrawLocalHours(Image image, TransformParams tParams)
        {
            ...
            for (int i = 0; i < nx; i++)
            {
                double lambda = (i - nx / 2) * 2 * Math.PI / nx + tParams.widgetLon;
                latPlotter.lambda = lambda;
                // Omit the last section of the lines of latitude.
                image.PlotLine(-ProjMath.Inclination, +ProjMath.Inclination, latPlotter, tParams.widgetColor, 4);
            }
        }
        ...
        void DrawGrid(Image image,
			          TransformParams tParams)
        {
            int gridx = tParams.gridX;
            int gridy = tParams.gridY;
            int nx = 360 / gridx;
            int ny = 180 / gridy;
            ...
            for (int i = 0; i < nx; i++)
            {
                ...
                double lambda = (i - nx / 2) * 2 * Math.PI / nx;
                latPlotter.lambda = lambda;
                // Omit the last section of the lines of latitude.
                //image.PlotLine(-pi/2+radians(gridy), pi/2-radians(gridy), latplotter, cmdtParams.color);
                image.PlotLine(-Math.PI / 2, Math.PI / 2, latPlotter, tParams.gridColor, 16);
            }
            ...
            for (int i = 0; i <= ny; i++)
            {
                ...
                image.PlotLine(-Math.PI, Math.PI, longPlotter, tParams.gridColor, 16);
            }
        }
        ...
        void DrawAnalemma(Image image, TransformParams tParams)
        {
            ...
            for (int i = 0; i < nx; i++)
            {
                double time = 2 * Math.PI * (i - nx / 2) / nx;
                analemmaPlotter.time = time;
                image.PlotLine(0, ProjMath.TwoPi, analemmaPlotter, tParams.widgetColor, 16);
            }
        }
        ...
        void DrawAltitudes(Image image, TransformParams tParams)
        {
            ...
            for (int i = 10; i <= 80; i += 10)
            {
                ...
                image.PlotLine(0, ProjMath.TwoPi, fooPlotter, tParams.widgetColor, 16);
            }
        }
				           ...
				           TransformParams tParams)
        {
            ...
            {
                tempPlotter.time = i;
                image.PlotLine(-ProjMath.Inclination, +ProjMath.Inclination, tempPlotter, tParams.widgetColor, 4);
            }
        }
			            ...
			            TransformParams tParams)
        {
            ...
            image.PlotLine(-Math.PI, Math.PI, longPlotter, tParams.widgetColor, 16);
        ...
        void DrawDatetime(Image image, TransformParams tParams)
        {
            ...
            double time = (day % 1.0d) - 0.5; // Time relative to noon
            double date = 2 * Math.PI * day / 365;
            double phi = ProjMath.SunDec(date);
            double eot = ProjMath.EquationOfTime(date) / (24 * 60 * 60);
            double apparenttime = time + eot; // AT = MT + EOT
            double lambda = -(2 * Math.PI * apparenttime);
            double x = 0.0, y = 0.0;
